...
def main():
   ...
   #     json_data.close()
    ...
    #TOKEN = api_config['1212212212121212']
   ...
   #     tb.reply_to(message, "Welcome to BinancePump Bot, Binance Top Tick Count, Top Price and Volume Change Feeds will be shared with you. One of it could be start of pump or dump, keep an eye on me!")
                ...
                for s in range(show_limit):
                    ...
                    if (s<len(sorted_price_group)):
                        ...
                        if not max_price_group.isPrinted:
                            ...
                            print('I have ' + str(max_price_group.symbol))
                            symbol2 = str(max_price_group.symbol)
                            print(symbol2)
                            lp2 = float(max_price_group.last_price)
                            lp3 = float(max_price_group.last_price)*2/100+float(max_price_group.last_price)
                            lp4 = float("{:.4f}".format(lp3))
                            print(lp2)
                            print(lp4)
                            print('I have ' + str(max_price_group.relative_price_change))
                            btc_price = str(client.get_symbol_ticker(symbol='BTCUSDT'))
                            rpc2 = str(max_price_group.relative_price_change)
                            balance2 = str(client.get_asset_balance('USDT')['free'])
                            print(balance2)
                            quantity = (float(balance2) / float(lp2)) - 1
                            quantity4 = (float(balance2) / float(lp2)) - 5
                            quantity2 = round(quantity)
                            quantity5 = round(quantity4)
                            quantity3 = str(quantity2)
                            quantity6 = str(quantity5)
                            print('quantity ' + quantity3)
                            print('quantity ' + quantity6)
                            if (float(rpc2)>5 and float(quantity3)>30):
                             buy_limit = client.order_limit_buy(symbol=symbol2, quantity=quantity3, price=lp2)
                             print('aldi')
                             quantity7 = str(quantity6)
                             time.sleep(30)
                            # if  (float(rpc2)>2):
                             print('quantity ' + quantity6)
                             sell_limit = client.order_limit_sell(symbol=symbol2, quantity=quantity6, price=lp4)
                            #  market_order = client.order_market_sell(symbol=symbol2, quantity=quantity6)
                             print('Satis Emri')

                            # x = 1
                            # if (x == 1 and float(rpc2)<13):
                            #  sell_limit = client.order_limit_sell(symbol=symbol2, quantity=quantity3, price=lp3)
                            #  print('karda satti')
                            # elif (float(rpc2)<8)
                            #  market_order = client.order_market_sell(symbol=symbol2, quantity=quantity3) 
                            # print('Stop loss')
                           # else:
                           # print('thanks')
                           # print(test)
    ...
    client = Client("JYDqQqGDA9L7yNYvm0maNftlrvT0FPZp9V1Y6sEoz73ewuuNm6cQsCTwwqLShicb", "D0KxKqn942zUaB3UdLIFRzc93fdw4s9BRaEMX9m0Qz8mWSjlA6hGTDyPk8bAK3OK")
